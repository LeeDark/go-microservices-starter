# gRPC Playground — учебные заметки по Go, gRPC и Protobuf
...
Buf подключается после того, как protobuf-контракт уже понятен.

- `buf.yaml` — локальный module, lint и breaking-конфигурация;
- `buf.gen.yaml` — локальные Go-плагины и настройки generated output;
- `buf format` — единое форматирование protobuf;
- `buf lint` — проверки контракта и стиля;
- `buf breaking` — проверки совместимости версий контракта;
- `buf generate` — воспроизводимая генерация Go-кода.

Текущие `.proto` не имеют protobuf-импортов, поэтому внешних Buf-зависимостей и `buf.lock` пока нет.
Существующий workflow `protoc` + Makefile остаётся доступным для сравнения и как fallback.

Breaking checks сравнивают разные ревизии одного package. Versioned packages `catalog.v1` и
`catalog.v2` являются разными историями API, поэтому эксперимент выполнялся на временных fixture с
одним package: добавление поля прошло, а изменение типа и удаление поля были отклонены.

Генерация Buf функционально эквивалентна текущему output Makefile/protoc. Единственное наблюдаемое
отличие — комментарий с версией protoc в generated header (`v7.35.1` и `(unknown)`).
